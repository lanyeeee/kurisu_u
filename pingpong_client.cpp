...
{
    printf("total readcount=%llu readbytes=%llu\n", (unsigned long long)total_readcount, (unsigned long long)total_readbytes);
...
int main(int argc, char** argv)
{
    ...
    printf("[%s:%d] %d threads %d connections %d byte  run %ds\n", host, port, threads, connections, sendbytes, seconds);
    ...
    // stop after seconds
    loop_threads.loop()->setTimeout(seconds * 1000, [&loop_threads](TimerID timerID) { loop_threads.stop(false); });
